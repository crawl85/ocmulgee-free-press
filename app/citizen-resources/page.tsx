...
export default function ResourcesPage() {
  return (
    <main><PageIntro eyebrow="Rights you can use" title="Citizen Empowerment" description="Plain-language tools for requesting records, attending meetings, understanding public spending, and holding officials to their obligations." />
      <section className="shell resource-grid">{resources.map((resource, index) => <article key={resource.title}><span className="resource-index">0{index + 1}</span><p className="kicker">{resource.type}</p><h2>{resource.title}</h2><p>{resource.description}</p><a className="resource-action" href={resource.href} download={resource.download || undefined} target={resource.external ? "_blank" : undefined} rel={resource.external ? "noopener noreferrer" : undefined}>{resource.action} <span aria-hidden="true">→</span></a></article>)}</section>
